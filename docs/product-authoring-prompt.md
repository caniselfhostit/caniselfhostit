...
- **Byte bands**: `prompt.md` 10,000–15,000 bytes (hard ceiling 16,500). `prompt-chat.md` 1.1–1.6× of prompt.md. `prompt-local.md` 9,000–15,000. The previous batch shipped at ~6.5k / 0.30× / ~6.8k — half depth with a vestigial chat path. The bands are not padding targets: fill them with product-specific substance (failure modes, upgrade path, the product's central handoff).
  - **Third-artifact exception** (precedent: OpnForm, batch 11 — shipped at 17,694 / 20,147): an entry whose stack structurally REQUIRES embedded config artifacts beyond compose + Caddyfile (e.g. a FastCGI-only app image forcing an in-network ingress config) may exceed the 16,500 ceiling by the byte size of those extra artifacts in each affected path; the local path's allowance additionally covers its canonical Docker block and the larger local compose it must embed. The necessity must be proven in the entry's record (block 9 / deviations), not chosen for convenience; the extra artifact obeys the same byte-identical parity rule across every copy; and prose gets no extra allowance — it must sit at its zero-cost floor (verifier-adjudicated), which is what the precedent's numbers represent.
